  ...
  return (
    <Link to={`/store/${store.id}`}
      ...
      <div className="h-32 relative overflow-hidden" style={{ background: grad }}>
        {store.logo
          ? <img src={store.logo} alt={store.storeName || store.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          : (
      ...
      <div className="px-4 pb-4 pt-2">
        <p className="font-bold text-gray-900 text-sm truncate">{store.storeName || store.name || 'متجر QuickMart'}</p>
