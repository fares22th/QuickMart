    ...
    <div className="bg-white border-b">
      ...
      <div className="max-w-7xl mx-auto px-4 pb-4 -mt-10 relative">
        <div className="w-20 h-20 rounded-2xl bg-white shadow-md overflow-hidden border-2 border-white">
          {store?.logo && <img src={store.logo} alt={store?.storeName || store?.name} className="w-full h-full object-cover" />}
        </div>
        <h1 className="text-2xl font-bold mt-2">{store?.storeName || store?.name}</h1>
