...
import { createProduct, updateProduct, uploadProductImages } from '@/api/products.api'
...
const inp = (err) =>
  `w-full border rounded-xl px-4 py-2.5 text-sm outline-none transition-all
   ${err ? 'border-red-400 focus:ring-2 focus:ring-red-100' : 'border-gray-200 focus:border-[#00C896] focus:ring-2 focus:ring-[#00C896]/10'}`
...
export default function ProductForm({ product, onSuccess }) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, reset } = useForm({
    ...
    defaultValues: product
      ...
      : { discount: 0, stock: 0, unit: 'قطعة', is_new: false, is_featured: false },
  })

  useEffect(() => { if (product) { setKeptUrls(product.images ?? []); setTags(product.tags ?? []) } }, [product])

  const mutation = useMutation({
    mutationFn: async (data) => {
      if (!store?.id) throw new Error('لم يتم العثور على متجرك')

      let imageUrls = [...keptUrls]
      if (newFiles.length) {
        const uploaded = await uploadProductImages(product?.id ?? 'temp-' + Date.now(), newFiles)
        imageUrls = [...imageUrls, ...uploaded]
      }

      const payload = { ...data, images: imageUrls, tags, store_id: store.id }
  ...
  return (
    <form onSubmit={handleSubmit(d => mutation.mutateAsync(d))}
      ...
      <Field label="اسم المنتج" error={errors.name?.message}>
        <input placeholder="مثال: تفاح أحمر طازج ١ كيلو"
          className={inp(errors.name)} {...register('name')} />
      </Field>

      {/* Description */}
      <Field label="وصف المنتج" error={errors.description?.message}
        hint="اشرح مميزات المنتج، مكوناته، طريقة الاستخدام...">
        <textarea rows={4} placeholder="أضف وصفاً تفصيلياً يساعد المشتري على اتخاذ قراره..."
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {tags.map(t => (
            <span key={t}
              className="flex items-center gap-1 bg-[#E6FAF5] text-[#00A878] text-xs font-medium px-3 py-1 rounded-full">
        ...
        <div className="flex gap-2">
          <input value={tagInput} onChange={e => setTagInput(e.target.value)}
            ...
            className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-[#00C896]" />
        ...
        {[
          ...
          <label key={name} className="flex items-center gap-3 cursor-pointer group">
            <div className="relative">
              ...
              <div className="w-10 h-6 bg-gray-200 peer-checked:bg-[#00C896] rounded-full transition-colors" />
      ...
      <button type="submit" disabled={mutation.isPending || isSubmitting}
        className="w-full py-3.5 rounded-2xl text-white font-bold text-sm transition-all duration-200 flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-60"
        style={{ background: 'linear-gradient(135deg,#00C896,#00A878)', boxShadow: '0 8px 24px rgba(0,200,150,.3)' }}>
        {mutation.isPending
