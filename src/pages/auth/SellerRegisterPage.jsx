...
const s3 = z.object({
  crNumber:    z.string().length(10, 'رقم السجل التجاري يجب أن يكون ١٠ أرقام بالضبط'),
  nationalId:  z.string().length(10, 'رقم الهوية يجب أن يكون ١٠ أرقام بالضبط'),
  sellerTerms: z.literal(true, { errorMap: () => ({ message: 'يجب الموافقة على شروط البائعين' }) }),
})
...
/* ── Progress bar ── */
const STEPS = ['معلوماتك','المتجر','الوثائق']
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className={bx(errs.phone?.message)}>
          <div className="flex items-center gap-1.5 px-3 py-3 bg-gray-50 border-l border-gray-200 shrink-0">
            <span className="text-base leading-none">🇸🇦</span>
            <span className="text-sm font-medium text-gray-600">+966</span>
          </div>
          <input type="tel" placeholder="5xxxxxxxx" dir="ltr" className={ic} {...reg('phone')} />
        </div>
        <Err msg={errs.phone?.message} />
      </div>
...
function Step3({ reg, errs }) {
  return (
    <div className="space-y-4">
      <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 flex gap-3">
        <span className="text-xl shrink-0">📋</span>
        <div>
          <p className="text-sm font-bold text-amber-800">وثائق رسمية مطلوبة</p>
          <p className="text-xs text-amber-600 mt-0.5 leading-relaxed">مطلوبة للتحقق من هويتك وتفعيل حسابك التجاري</p>
        </div>
      </div>
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-2">رقم السجل التجاري</label>
        <div className={bx(errs.crNumber?.message)}>
          <div className="px-3 text-gray-400"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg></div>
          <input type="text" placeholder="١٠١٠XXXXXX" dir="ltr" maxLength={10} className={ic + ' font-mono tracking-wide'} {...reg('crNumber')} />
        </div>
        <Err msg={errs.crNumber?.message} />
      </div>
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-2">رقم الهوية الوطنية</label>
        <div className={bx(errs.nationalId?.message)}>
          <div className="px-3 text-gray-400"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0" /></svg></div>
          <input type="text" placeholder="1XXXXXXXXX" dir="ltr" maxLength={10} className={ic + ' font-mono tracking-wide'} {...reg('nationalId')} />
        </div>
        <Err msg={errs.nationalId?.message} />
      </div>
      <label className="flex items-start gap-2.5 cursor-pointer pt-1">
        <input type="checkbox" className="w-4 h-4 mt-0.5 rounded shrink-0" style={{ accentColor: '#F59E0B' }} {...reg('sellerTerms')} />
        <span className="text-sm text-gray-600 leading-relaxed">
          أوافق على{' '}<a href="#" className="font-semibold text-amber-600">شروط البائعين</a>{' '}و{' '}<a href="#" className="font-semibold text-amber-600">سياسة العمولات</a>
        </span>
      </label>
      <Err msg={errs.sellerTerms?.message} />
    </div>
  )
}
...
export default function SellerRegisterPage() {
  ...
  const onSubmit = async (data) => {
    await registerUser({ ...saved, ...data, role: 'seller' })
    navigate('/verify-otp')
  }
  ...
  const titles = [null,
    ...
    { h: 'الوثائق الرسمية',    s: 'خطوة أخيرة للتحقق منك' },
  ]
        ...
        <div className="flex-1 flex items-center justify-center px-6 sm:px-14 py-10">
          <div className="w-full max-w-sm">
            ...
            <form onSubmit={handleSubmit(isLast ? onSubmit : onNext)}>
              ...
              {step === 3 && <Step3 reg={register} errs={errors} />}
